...
def derivative(coeff):			# returns coefficients of derivative of a polynomial
	n = len(coeff) - 1
	dcoeff = []
	for i in range(n):
...
def poly_str(coeff):			# Shows a polynomial neatly
    n = len(coeff) - 1
    terms = []
    for i in range(len(coeff)):
...
def Simpson_Int(f, a, b, N=10000):
	N = N if N % 2 == 0 else N + 1		# Simpson requires even N
	h = (b-a)/N
	s = f(a)+f(b)
	for i in range(1, N):
		x = a + i*h
...
def Gaussian_Int(f, a, b, n=5):		# Gauss-Legendre for finite [a,b]; Gauss-Laguerre when b=float('inf')
	legendre = {		# Legendre nodes and weights on [-1, 1] for n = 1 to 5
		1: ([0.0],
			[2.0]),
		2: ([-0.577350269, 0.577350269],
			[1.0, 1.0]),
		3: ([-0.774596669, 0.0, 0.774596669],
			[0.5556, 0.8889, 0.5556]),
		4: ([-0.861136312, -0.339981044, 0.339981044, 0.861136312],
			[0.3478, 0.6521, 0.6521, 0.3478]),
		5: ([-0.906179846, -0.538469310, 0.0, 0.538469310, 0.906179846],
			[0.2369, 0.4786, 0.5689, 0.4786, 0.2369])
	}

	laguerre = {		# Laguerre nodes and weights for n = 1 to 5
		1: ([1.0],
			[1.0]),
		2: ([0.585786438, 3.414213562],
			[0.8535, 0.1464]),
		3: ([0.415774456, 2.294280360, 6.289945082],
			[0.7111, 0.2785, 0.0104]),
		4: ([0.322547689, 1.745761101, 4.536620297, 9.395070912],
			[0.6032, 0.3574, 0.0389, 0.0005]),
		5: ([0.263560320, 1.413403059, 3.596425772, 7.085191860, 12.640800844],
			[0.5217, 0.3987, 0.0759, 0.0036, 0.0000])
	}

	if b == float('inf'):				# Gauss-Laguerre branch; f is passed as e^-x * g(x), so we extract g(x) = f(x)*e^x
		nodes, weights = laguerre[n]
		return sum(weights[i] * f(nodes[i]) * math.exp(nodes[i]) for i in range(n))
									# e^-x weight is built into the quadrature; we undo e^-x from f to get bare g(x)
	else:								# Gauss-Legendre branch; change of variable from [a,b] to [-1,1]
		nodes, weights = legendre[n]
		mid   = (b + a) / 2.0			# (b+a)/2 shift
		half  = (b - a) / 2.0			# (b-a)/2 scale
		return half * sum(weights[i] * f(half * nodes[i] + mid) for i in range(n))
