...
def LU(A):   #DoLittle Used; overwrites og matrix with L and U. L[i,i] are to be read implicitly
	n = len(A)
	for i in range(n):
...
def forward(A, B): #LU-decomposition forward substituition
	n = len(A)
	y = [0.0]*n
	for i in range(n):
...
def backward(A, y): #LU-decomposition backward substituition
	n = len(A)
	x = [0.0]*n
...
def Cholesky(A):	# for HPD matrices; overwrites lower triangle of A with L
	n = len(A)
	for i in range(n):
		for j in range(n):
			if A[i][j] != A[j][i]:
